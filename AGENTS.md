...
- `src/ultron/core/tools/` — `registry.py` maps tool names → functions (38 tools) and generates the JSON Schema the ReAct prompt uses. Add a tool: function in `tools/builtin/`, register it, optionally a `detect_*`/`handle_*` pair in `simple.py`. Memory lives in `tools/memory/` (graph triples + sqlite flat facts).
